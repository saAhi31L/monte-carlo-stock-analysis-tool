...
showMarketContext = False # Pre-initialized so it is never unbound if an early exception fires
...
try:
    ...
    #Ask whether the user wants the broader market-context sections (regime, sentiment,
    #commodities, sector fear index) alongside the core stock report, or just the stock itself
    marketContextInput = input("Show broader market context (regime, sentiment, commodities, sector fear index)? (y/n): ").strip().lower()
    showMarketContext = marketContextInput in ("y", "yes")
    print()
...
print(f"\n  P/E Ratio:       ${peStr}  ({valuation})")
print(f"  Forward P/E:     ${forwardPEStr}")
print(f"  EPS Growth:      ${epsGrowthStr}")
print(f"  D/E Ratio:       ${debtToEquityStr}")
print(f"  P/B Ratio:       ${priceToBookStr}")
print(f"  52-Week High:    ${fiftyTwoWeekHigh}")
print(f"  52-Week Low:     ${fiftyTwoWeekLow}")
print(f"  Turnover Rate:   {turnoverStr}")

#-------------------------------------------------------------------------------------------------#
# NEW: Investment Horizon Fit (ticker-specific -- reuses closeData, epsGrowthValue, peRatio, debtToEquityRatio)
try:
...
#-------------------------------------------------------------------------------------------------#
# NEW: Broader market context -- Sector Fear Index, Market Regime, Market Sentiment, Commodities.
# All four are gated behind the user's y/n answer to showMarketContext, since this is market-wide
# context rather than information about the specific stock being analyzed.
if showMarketContext:
    try:
        ...
        print("\n  Sector Fear Index: Unavailable (data fetch failed)")

    # Shared batched fetch for Regime + Sentiment
    try:
    ...
    try:
        ...
        for c in commodities:
            ...
            commodityRows.append([c["name"], c["ticker"], priceStr, changeStr])
        commodityTable = tabulate(
            commodityRows,
            headers=["Commodity", "Ticker", "Price", "1-Day Change"],
            tablefmt="fancy_grid", numalign="right", stralign="center",
...
result = subprocess.run(["pixi", "run", "mojo", "MonteCarloRiskEngine.mojo"])
