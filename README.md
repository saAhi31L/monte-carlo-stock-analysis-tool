...
alongside fundamentals (each tagged with a plain-language read, e.g. "33.26x
(Over-Saturated)"), an Earnings Watch caveat, and optional broader market context
(sentiment, commodities, market regime, sector fear index). Every run's predictions
are logged locally so their real-world accuracy can be checked later.
